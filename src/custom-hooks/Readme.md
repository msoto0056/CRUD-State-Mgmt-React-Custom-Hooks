...
###### All you need is one line of code for each CRUD Operation
###### and the package will use the magic provided by React Query 
###### to perform each HTTP operation using axios.
###### React Query was created by Tanner Linsley. 
...
#### State Managment 
* The State Managment Hooks provide a Redux like Store using React Hooks available since v16.8. There are 2 versions MakeStore and MakeStore2. both of them return an Store provider, an Store and a Dispatch function. The difference is that MakeStore2 returns a tuple [Store,Dispatch] to follow regular useReducer format and the MakeStore returns separete Store and Dispatch components to facilitat it usage in the useEffect dependency array and reduce the # of renderings.*
...
##### then; Wrap elements that need the server state  in 
...
###### Then inside a React functional Component - since it's a hook, y must follow hook rules
...
###### the return
    ...
    * data that is the information fetch from the server for the specified url.
...
###### const  const {isLoading:loadingDel, remove, error:errorDel,  isError:isErrorDel} = useDelete("tasks",url,onSuccess(optional)) 
